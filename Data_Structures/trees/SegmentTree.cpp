...
ll build(int ind, int l, int r) {
    ...
    int mid = (l + r) >> 1;
    return segTree[ind] = build(left(ind), l, mid) +
...
ll query(int ind, int l, int r, int A, int B) {
    ...
    if(r < A || l > B) return 0;
    int mid = (l + r) >> 1;
    return query(left(ind), l, mid, A, B) +
            query(right(ind), mid+1, r, A, B);
}
...
ll update(int ind, int l, int r, int x, int val) {
    ...
    int mid = (l + r) >> 1;
    if(x <= mid) update(left(ind), l, mid, x, val);
    else update(right(ind), mid+1, r, x, val);
    return segTree[ind] = segTree[left(ind)] + segTree[right(ind)];
}
